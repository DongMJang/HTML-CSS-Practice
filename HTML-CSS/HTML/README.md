...
# srcset
>브라우저에 사용할 이미지들과 그 이미지들의 **원본 크기를 지정**
>이미지 크기는 **px이 아닌** **w 또는 x 단위**로 입력
>**작은 크기 이미지부터** 순서대로 입력

* ## W unit
>가로 너비를 의미 (Width descriptor) 
고정할려면 width 추가

* ## X unit
>이미지 비율을 의미 (Device pixel ratio descripotr)
대부분 w 단위를 사용

# sizes
> 미디어조건과 그 조건에 해당하는 이미지의 최적화 출력 크기를 지정
